...
// Asignar vehículos a entregas
void assign_vehicles_to_deliveries(Delivery *deliveries, int n_deliveries, Vehicle *vehicles, int n_vehicles) 
{
    ...
    for (int i = 0; i < n_deliveries; i++) {
        ...
        if (best_vehicle != -1) {
            ...
            float wait_time = (delivery_start_time > vehicle_arrival_time) ? 
                              (delivery_start_time - vehicle_arrival_time) : 0;
            ...
            // Reducir la capacidad del vehículo
            vehicles[j].capacity_volume -= deliveries[i].volume;
            vehicles[j].capacity_weight -= deliveries[i].weight;

            // Actualizar la posición del vehículo al destino de la entrega
    ...
    clock_t end_time = clock(); // Finaliza el temporizador
