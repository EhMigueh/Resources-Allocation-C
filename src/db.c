...
void generate_random_deliveries_csv()
{
    ...
    for (int i = 0; i < MAX_NUM_DELIVERIES; i++)
    {
        char id[MAX_ID_LENGTH];
        char start[7];
        char end[7];
        sprintf(id, "D%03d", i + 1);
    ...
    fclose(file);
    fprintf(stdout, GREEN_COLOR "\nArchivo deliveries.csv creado con éxito.\n" RESET_COLOR);
}
...
void generate_random_vehicles_csv()
{
    ...
    fclose(file);
    fprintf(stdout, GREEN_COLOR "Archivo vehicles.csv creado con éxito.\n\n" RESET_COLOR);
}

void create_random_databases()
{
    generate_random_deliveries_csv();
    generate_random_vehicles_csv();
    exit(EXIT_SUCCESS);
}